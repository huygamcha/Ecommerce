...
import { createCartFromCustomer, getAllCart, resetCartNotification } from '../../slices/cartSlice'
...
function HeaderScreen() {
  ...
  const handleSearchMenu = (categoryId: string, brandId: string) => {
    localStorage.setItem('filter', JSON.stringify({ categoryId: categoryId, brandId: brandId }))
  }
    ...
    <>
      ...
      >
        <Row justify="end" className={clsx(style.wrapper_try)}>
              ...
              <Row style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Col xs={20} sm={0}>
                  <Link to="/" className={clsx(style.header_image)}>
                    <img src="https://pub-50bb58cfabdd4b93abb4e154d0eada9e.r2.dev/fff.png" alt="" />
                  </Link>
                </Col>
                <Col xs={2} sm={0}>
                  <Link
                  ...
                  >
                    <div className={clsx(style.button_header_text)}>
                      <FaCartShopping className={clsx(style.cart_notification_wrapper)} />
                        ...
                        productsSearch.map((product, index) => (
                          ...
                          >
                            <Flex>
                              ...
                              <Flex vertical>
                                <div className={clsx(style.header_name)}>{product.name}</div>
                                <Space style={{ fontWeight: 'bold' }}>
                                  <div className={clsx(style.header_price)}>
                                    {' '}
                                    {numeral(product.total).format('0,0$')} / {product.unit}
                                  </div>
                                </Space>
                              </Flex>
                            </Flex>
            ...
            <Flex justify="end">
              {currentUser ? (
                <Space className={clsx(style.profile_detail)}>
                  <Space style={{ marginRight: '14px' }} className={`${style.button_header}`}>
                    <Link className={clsx(style.button_header_text)} to="/profile">
                  ...
                  <Flex className={clsx(style.profile_detail_child)}>
                    <Space>
                      <Link style={{ color: '#000', fontWeight: 'normal' }} to="/profile">
                        <UserDeleteOutlined />
                        <Space style={{ marginLeft: '5px' }}>Thông tin cá nhân</Space>
                      </Link>
                    </Space>
                    <Space onClick={handleLogout} style={{ color: '#000', fontWeight: 'normal' }}>
                      <LogoutOutlined />
                ...
                <>
                  <Space style={{ marginRight: '14px' }} className={clsx(style.button_header)}>
              ...
              <Space className={clsx(style.button_header, style.button_background)}>
                <Link onClick={handleCart} className={clsx(style.button_header_text)} to="/cart">
          ...
          <Space>
            {isOpen && (
              <div onClick={(e) => e.stopPropagation()} className={clsx(style.menu_mobile_wrapper)}>
                <Flex ref={menuMobileRef} className={clsx(style.menu_mobile)} vertical>
                  ...
                  <Row>
                    <Col span={24}>
                      <Flex className={clsx(style.auth_wrapper_mobile)} vertical>
                        <Space className={clsx(style.auth_wrapper_mobile_text)}>
                          Đăng nhập để hưởng những đặc quyền dành riêng cho hội viên
                        </Space>
                        <Flex justify="space-between">
                          <Space className={clsx(style.add_to_cart)}>Đăng nhập</Space>
                        ...
                        categories.map((category, index) => (
                          <div key={index}>
                            ...
                            >
                              ...
                              >
                                <div style={{ width: '70%' }} onClick={() => setIsOpen(false)}>
                                  {category.name}
                                </div>
                              </Link>
                  ...
                  <a href="https://zalo.me/0933110500">
                    <Flex className={clsx(style.contactNowMobile_wrapper)}>
                      <Flex className={clsx(style.contactNowMobile)}>
                        ...
                        <Space className={clsx(style.contactNowMobile_phone_text)}>
              ...
              categories.map((category, index) => (
                <React.Fragment key={index}>
                  ...
                  >
                    ...
                    <Flex vertical className={clsx(style.brand_list, style.brand_list_mobile)}>
                      {brands.map((brand, index) => {
                        if (
