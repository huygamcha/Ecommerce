...
const getCategoryByName = createAsyncThunk<CategoriesType, string | undefined>(
  ...
  async (name) => {
    const response = await axios.get(`${process.env.REACT_APP_BACKEND}/categories/name/${name}`)
    const data: CategoriesType = response.data.payload
...
const updateCategory = createAsyncThunk<CategoriesType, { id: string; values: CategoriesType }>(
...
export { getAllCategory, createCategory, deleteCategory, updateCategory, getCategoryByName }
